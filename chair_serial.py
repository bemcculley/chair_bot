                ...
                for thing in event_list:
                    ...
                    if thing.type == 10:
                        if thing.dict['button'] == 0: # A button
                            ...
                            time.sleep(5)
                            #self.handle_routine()
                        elif thing.dict['button'] == 11:
                            print 'got it'
                            send_to_hat('animate', 1)
                        elif thing.dict['button'] == 12:
                            send_to_hat('animate', 2, 50)
                        print 'xx'
                        print thing.dict['button']
                        print 'xx'
...
def send_to_hat(led_type, vals, delay=10):
    hat = True
    if not hat:
        return
    try:
        subprocess.call('curl http://192.168.42.2/led?%s=%s&delay=10' % (led_type, vals), shell=True)
    except Exception, e:
        print 'Error! %s' % e
